...
with st.sidebar:
    ...
    st.markdown(
        ...
        <a href="#section-delta" class="nav-link">📈 Чувствительность к δ</a>
        """,
        unsafe_allow_html=True,
    )
...
def sweep_delta_metrics(
    ...
    """Серия BL-расчётов по сетке δ → агрегаты, доходности и веса по тикерам."""
    cov = pickle.loads(cov_bytes)
    w_mkt = pickle.loads(w_bytes)
    pred_local = pickle.loads(pred_bytes)
    ...
    for d in deltas:
        res = run_black_litterman(
            tickers, cov, w_mkt, views, delta=float(d), tau=tau_f, long_only=True
        )
        pi, mu, w = res.pi, res.mu_bl, res.weights_bl
        erp_mkt = float(w_mkt @ pi)
        mu_p = float(w @ mu)
        ...
        summary_rows.append(
            {
                "delta": float(d),
                "erp_market_pct": erp_mkt * 100,
                "pi_mean_excess_pct": float(pi.mean() * 100),
                "pi_max_abs_pct": float(np.max(np.abs(pi)) * 100),
                "mu_mean_excess_pct": float(mu.mean() * 100),
                "mu_max_shift_pp": float(np.max(np.abs(shift)) * 100),
                "mu_mean_shift_pp": float(np.mean(np.abs(shift)) * 100),
                "portfolio_excess_pct": mu_p * 100,
                "portfolio_vol_pct": sigma_p * 100,
                "sharpe": mu_p / sigma_p if sigma_p > 1e-12 else float("nan"),
                "max_weight_pct": float(w.max() * 100),
                "hhi": float(np.sum(w**2)),
                "turnover": float(np.abs(w - w_mkt).sum() / 2),
                "tracking_error_pct": float(
                    np.sqrt((w - w_mkt) @ cov @ (w - w_mkt)) * 100
                ),
            ...
            return_rows.append(
                {
                    "delta": float(d),
                    "ticker": t,
                    "pi_excess_pct": float(pi[i] * 100),
                    "mu_excess_pct": float(mu[i] * 100),
                    "shift_pp": float(shift[i] * 100),
                }
            )
            weight_rows.append(
                {
                    "delta": float(d),
                    "ticker": t,
                    "weight_bl_pct": float(w[i] * 100),
                    "weight_mkt_pct": float(w_mkt[i] * 100),
                    "delta_weight_pp": float((w[i] - w_mkt[i]) * 100),
                }
            )
...
def plot_delta_lines(
    ...
    fig.add_vline(
        ...
        annotation_text=f"δ={delta_current:g}",
        annotation_position="top right",
    )
    fig.update_layout(
        title=title,
        xaxis_title="δ — штраф за риск",
        height=height,
        margin=dict(t=50, b=40),
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="left", x=0),
        hovermode="x unified",
...
def plot_delta_ticker_lines(
    ...
    sub = df[df["ticker"].isin(tickers_subset)]
    fig = px.line(
        sub,
        x="delta",
        y=value_col,
...
# ═════════════════════════════════════════════════════════════════════════════
# РАЗДЕЛ 7: ЧУВСТВИТЕЛЬНОСТЬ К δ
# ═════════════════════════════════════════════════════════════════════════════
st.markdown('<span id="section-delta" class="section-anchor"></span>', unsafe_allow_html=True)
st.markdown("## 📈 Анализ чувствительности к δ")

st.markdown(
    """
    <div class="section-card">
    Параметр <b>δ</b> (штраф за риск) влияет на prior <b>π = δΣw</b>, posterior <b>μ̂</b>
    и оптимальные веса <b>w*</b>. Ниже — как меняются ключевые величины при сканировании δ
    (τ и Rf фиксированы; views и Ω не зависят от δ).
    Красная линия — текущее значение δ из боковой панели.
    </div>
    """,
    unsafe_allow_html=True,
)
...
with dc1:
    delta_sweep_min = st.number_input(
        "δ min (сетка)",
        min_value=0.1,
        value=0.5,
        step=0.5,
...
with dc2:
    delta_sweep_max = st.number_input(
        "δ max (сетка)",
        min_value=0.5,
        value=25.0,
        step=0.5,
...
if delta_sweep_max <= delta_sweep_min:
    st.error("δ max должен быть больше δ min.")
else:

    @st.cache_data(show_spinner="Сканирование δ…")
    def _cached_delta_sweep(
        tickers_key: tuple,
        cov_bytes: bytes,
        w_bytes: bytes,
        pred_bytes: bytes,
        ...
        return sweep_delta_metrics(
            tickers_key, cov_bytes, w_bytes, pred_bytes, tau_f, rf_f, d_min, d_max, n_pts
        )

    sum_df, ret_df, wgt_df = _cached_delta_sweep(
        tickers_key=tuple(tickers),
        cov_bytes=pickle.dumps(cov_np),
        w_bytes=pickle.dumps(w_mkt),
        pred_bytes=pickle.dumps(pred_df),
        tau_f=float(tau),
        rf_f=float(rf),
        d_min=float(delta_sweep_min),
        d_max=float(delta_sweep_max),
        n_pts=int(delta_sweep_n),
    )

    # Тикеры с наибольшим |Δw| при δ, ближайшем к текущему значению в sidebar
    snap_delta = float(sum_df.loc[(sum_df["delta"] - delta).abs().idxmin(), "delta"])
    snap_w = wgt_df[wgt_df["delta"] == snap_delta].copy()
    top_tickers = (
        snap_w.assign(abs_dw=snap_w["delta_weight_pp"].abs())
        .nlargest(6, "abs_dw")["ticker"]
        .tolist()
    )

    st.markdown("### Prior и posterior (доходности)")
    p1, p2 = st.columns(2)
    with p1:
        st.plotly_chart(
            plot_delta_lines(
                sum_df,
                [
                    ("erp_market_pct", "ERP рынка w'π"),
                    ("pi_mean_excess_pct", "Среднее π (excess)"),
                ],
                delta,
                "Prior: премия рынка и уровень π",
            ),
    ...
    with p2:
        st.plotly_chart(
            plot_delta_lines(
                sum_df,
                [
                    ("mu_mean_excess_pct", "Среднее μ̂ (excess)"),
                    ("portfolio_excess_pct", "Excess портфеля w'μ̂"),
                ],
                delta,
                "Posterior и доходность BL-портфеля",
            ),
            use_container_width=True,
        )
    ...
    with p3:
        st.plotly_chart(
            plot_delta_lines(
                sum_df,
                [
                    ("mu_max_shift_pp", "max |μ̂−π|"),
                    ("mu_mean_shift_pp", "mean |μ̂−π|"),
                ],
                delta,
                "Сдвиг views: насколько μ̂ отличается от π",
            ),
            use_container_width=True,
        )
    with p4:
        st.plotly_chart(
            plot_delta_lines(
                sum_df,
                [("pi_max_abs_pct", "max |π|")],
                delta,
                "Максимальная |π| по активам",
            ),
            use_container_width=True,
        )

    st.markdown("### Портфель: риск, доходность, структура")
    p5, p6 = st.columns(2)
    with p5:
        st.plotly_chart(
            plot_delta_lines(
                sum_df,
                [
                    ("portfolio_vol_pct", "Волатильность σ_p"),
                    ("sharpe", "Sharpe (excess/σ)"),
                ],
                delta,
                "Риск и Sharpe BL-портфеля",
            ),
            use_container_width=True,
        )
    with p6:
        st.plotly_chart(
            plot_delta_lines(
                sum_df,
                [
                    ("turnover", "Turnover vs рынок"),
                    ("tracking_error_pct", "Tracking Error, %"),
                ],
                delta,
                "Отклонение от рыночного портфеля",
            ),
            use_container_width=True,
        )
    ...
    with p7:
        st.plotly_chart(
            plot_delta_lines(
                sum_df,
                [
                    ("max_weight_pct", "Max вес, %"),
                    ("hhi", "HHI (концентрация)"),
                ],
                delta,
    ...
    st.markdown("### По тикерам (топ-6 по |Δw| при текущем δ)")
    st.caption(f"Тикеры: {', '.join(top_tickers)}")

    t1, t2 = st.columns(2)
    with t1:
        st.plotly_chart(
            plot_delta_ticker_lines(
                ret_df,
                "pi_excess_pct",
                top_tickers,
                delta,
    ...
    st.markdown("### Сводная таблица (выбранные δ)")
    showcase_deltas = sorted(
        {float(delta_sweep_min), 2.5, float(delta), 10.0, 20.0, float(delta_sweep_max)}
    )
    showcase_deltas = [d for d in showcase_deltas if delta_sweep_min <= d <= delta_sweep_max]
    table_cols = [
        "delta",
        "erp_market_pct",
        "portfolio_excess_pct",
        "portfolio_vol_pct",
        "sharpe",
        "mu_max_shift_pp",
        "max_weight_pct",
    ...
    showcase = sum_df[sum_df["delta"].isin(showcase_deltas)][table_cols].copy()
    showcase.columns = [
        "δ",
        "ERP рынка, %",
        "Excess портф., %",
    ...
    st.dataframe(
        showcase.round(3).sort_values("δ"),
        use_container_width=True,
        hide_index=True,
    )
    ...
    st.markdown(
        ...
        <div class="insight-box">
        <b>Как читать:</b><br>
        • <b>π и ERP рынка</b> растут линейно с δ (π = δΣw).<br>
        • <b>μ̂</b> тоже растёт, но views «оттягивают» к Q — сдвиг |μ̂−π| нелинеен.<br>
        • <b>Веса</b>: при большом δ prior сильнее → turnover и TE обычно снижаются,
        концентрация может расти.<br>
        • Текущее δ = <b>{delta:g}</b> отмечено красной линией на всех графиках.
        </div>
        """,
        unsafe_allow_html=True,
    )
